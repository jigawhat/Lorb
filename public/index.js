...
$( function() {
    ...
    $.getJSON( "champ_list.json", function( data ) {
        ...
        for (i = 0; i < champ_list.length; i++) {
            ...
            if (i > 0) {
                img_url = ddrag_url + champ_list[i][1] + '.png';
            }
        ...
        for (i = 0; i < champ_list.length; i++) {
            ch = champ_list[i];
            search_dropdown_inner += '<option value="' + ch[0] + '">' + ch[2] + '</option>';
        }
        ...
        var selected_indices = [...Array(10).keys()].map(i => 0); // Selected indices for each pl
        ...
        $( ".grid_button" ).click(function() { // Make grid appear
            ...
            current_i = selected_indices[ch_pl_i];
        ...
        $( "#champion_grid_cont" ).click(function() { // Make grid disappear
            ...
            var current_i = selected_indices[ch_pl_i];
        ...
        $( ".grid_img" ).click(function(event) { // Make grid disappear after setting new champion
            ...
            selected_indices[ch_pl_i] = sel_ind;
            req_data[ch_pl_i][cid_li] = champ_list[sel_ind][0];
            if (sel_ind > 0) {
                $( '#pl_' + ch_pl_i ).find( '.champion_box' ).css(
                    "background-image", 'url(' + ddrag_url + champ_list[sel_ind][1] + '.png)');
            } else {
                $( '#pl_' + ch_pl_i ).find( '.champion_box' ).css(
        ...
        $( ".search_button" ).click(function() {
            ...
            if (!dropdown_elem.hasClass("hide")) {
                ...
                // Set the current pl_i for editing the selected champion
                ch_pl_i = parseInt(pl_id[pl_id.length - 1]);
                $( ".search_select" )[0].selectedIndex = selected_indices[ch_pl_i];
